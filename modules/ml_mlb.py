...
def _frame_signature(df, important_columns):
    ...
    digest = hashlib.sha256(pd.util.hash_pandas_object(df[cols], index=True).values.tobytes()).hexdigest()
    return (int(len(df)), int(len(df.columns)), digest)


def _cache_key(df_batting, df_pitching, df_games):
    return (_frame_signature(df_batting, ('Team','Season','wRC+','OPS_Index','wOBA','ISO','K%','BB%')),
            _frame_signature(df_pitching, ('Team','Season','xFIP','FIP','ERA','K-BB%','WHIP','HR/9')),
...
def preferred_batting_column(df):
    ...
    if 'wRC+' in df.columns:
        ...
        source = df.get('wRC+_Source', pd.Series('', index=df.index)).astype(str)
        real = vals[source.str.contains('FANGRAPHS_REAL', case=False, na=False)]
        if real.notna().sum() >= max(20, int(vals.notna().sum() * 0.5)): return 'wRC+'
...
def preferred_pitching_column(df):
    ...
    if 'xFIP' in df.columns:
        ...
        source = df.get('xFIP_Source', pd.Series('', index=df.index)).astype(str)
        real = vals[source.str.contains('FANGRAPHS_REAL', case=False, na=False)]
        if real.notna().sum() >= max(20, int(vals.notna().sum() * 0.5)): return 'xFIP'
...
def _calibrate_sigma(pred, actual, market='spread'):
    ...
    if market=='spread':
        outcomes=[(actual+1.5>0).astype(float),(-actual+1.5>0).astype(float)]; projections=[pred+1.5,-pred+1.5]
    else:
        outcomes=[]; projections=[]
        for line in (7.5,8.5,9.5):
            outcomes += [(actual>line).astype(float),(actual<line).astype(float)]; projections += [pred-line,line-pred]
    for mult in np.linspace(.85,1.75,37):
        sigma=base*float(mult); scores=[]
        for y,margin in zip(outcomes,projections):
            probs=np.array([_normal_cdf(v/sigma) for v in margin]); scores.append(float(np.mean((probs-y)**2)))
...
class PredictorMLMLB:
    """Chronological pre-game model using rolling results + prior-season real team strength."""
    def __init__(self):
        self.modelo_ganador=self._new_classifier()
        self.modelo_carreras=GradientBoostingRegressor(n_estimators=140,max_depth=2,learning_rate=.035,loss='huber',random_state=42)
        self.modelo_handicap=GradientBoostingRegressor(n_estimators=140,max_depth=2,learning_rate=.035,loss='huber',random_state=42)
        self.entrenado=False; self.bat_scale=100.; self.pit_scale=4.10; self.current_history={}; self.current_h2h={}
        self.sigma_runs=3.5; self.sigma_diff=4.2; self.prob_shrink=1.; self.loaded_from_cache=False
        self.batting_metric=None; self.pitching_metric=None
    ...
    def _feature_row(self,hist,h2h,loc,vis,off_l,off_v,pit_l,pit_v):
        ...
        w20l,rf20l,ra20l,rd20l=team_state(hist,loc,20); w20v,rf20v,ra20v,rd20v=team_state(hist,vis,20); hwin,hrd,hn=h2h_state(h2h,loc,vis,12)
        return [w5l,w5v,w20l,w20v,rf5l,rf5v,ra5l,ra5v,rd5l,rd5v,rd20l,rd20v,hwin,hrd,min(hn,12)/12.,
                float(off_l)/max(self.bat_scale,1e-6),float(off_v)/max(self.bat_scale,1e-6),float(pit_l)/max(self.pit_scale,1e-6),float(pit_v)/max(self.pit_scale,1e-6),1.]
    @staticmethod
    ...
    def entrenar(self,df_batting,df_pitching,df_games):
        try:
            ...
            self.batting_metric, self.pitching_metric = bat_col, pit_col
            games=prepare_games(df_games); bd=self._stats_dict(df_batting,bat_col); pdict=self._stats_dict(df_pitching,pit_col)
            bv=pd.to_numeric(df_batting[bat_col],errors='coerce').dropna(); pvals=pd.to_numeric(df_pitching[pit_col],errors='coerce').dropna()
            self.bat_scale=float(bv.median()) if len(bv) else 100.; self.pit_scale=float(pvals.median()) if len(pvals) else 4.10
            X=[]; yw=[]; yr=[]; yd=[]; hist={}; hh={}
            for _,r in games.iterrows():
                ...
                X.append(self._feature_row(hist,hh,loc,vis,ol,ov,pl,pv)); yw.append(int(hs>as_)); yr.append(hs+as_); yd.append(hs-as_); append_game(hist,hh,loc,vis,hs,as_)
            ...
            temp=self._new_classifier(); temp.fit(X[:cut],yw[:cut]); raw=temp.predict_proba(X[cut:])[:,1]; best_alpha,best_brier=1.,float('inf')
            ...
            self.prob_shrink=best_alpha; self.modelo_ganador=self._new_classifier(); self.modelo_ganador.fit(X,yw); self.modelo_carreras.fit(X,yr); self.modelo_handicap.fit(X,yd)
            self.current_history,self.current_h2h=hist,hh; self.entrenado=True; self.loaded_from_cache=False
            cache={'modelo_ganador':self.modelo_ganador,'modelo_carreras':self.modelo_carreras,'modelo_handicap':self.modelo_handicap,'bat_scale':self.bat_scale,'pit_scale':self.pit_scale,
                   'sigma_runs':self.sigma_runs,'sigma_diff':self.sigma_diff,'prob_shrink':self.prob_shrink,'batting_metric':self.batting_metric,'pitching_metric':self.pitching_metric,
    ...
    def predecir_partido(self,loc_abbr,vis_abbr,wrc_loc,wrc_vis,xfip_loc,xfip_vis,pf=None):
        try:
            ...
            loc,vis=normalize_team(loc_abbr),normalize_team(vis_abbr); f=np.asarray([self._feature_row(self.current_history,self.current_h2h,loc,vis,float(wrc_loc),float(wrc_vis),float(xfip_loc),float(xfip_vis))],float)
            raw=float(self.modelo_ganador.predict_proba(f)[0,1]); pl=self._shrink_probability(raw,self.prob_shrink); runs=float(self.modelo_carreras.predict(f)[0]); diff=float(self.modelo_handicap.predict(f)[0])
            return {'Probabilidad_Local':round(pl*100,2),'Probabilidad_Visita':round((1-pl)*100,2),'Probabilidad_Local_Raw':round(raw*100,2),'Prob_Shrink':round(self.prob_shrink,3),
                    'Proyeccion_Carreras':round(runs,2),'Proyeccion_Handicap_Local':round(diff,2),'Sigma_Carreras':round(self.sigma_runs,3),'Sigma_Handicap':round(self.sigma_diff,3),
                    'Modelo_Desde_Cache':bool(self.loaded_from_cache),'Metrica_Bateo':self.batting_metric,'Metrica_Pitcheo':self.pitching_metric}
        except Exception as e:
            print(f'Error en predicción ML: {e}'); return {'Probabilidad_Local':50.,'Probabilidad_Visita':50.,'Probabilidad_Local_Raw':50.,'Prob_Shrink':1.,'Proyeccion_Carreras':8.5,'Proyeccion_Handicap_Local':0.,'Sigma_Carreras':3.5,'Sigma_Handicap':4.2,'Modelo_Desde_Cache':False,'Metrica_Bateo':None,'Metrica_Pitcheo':None}
