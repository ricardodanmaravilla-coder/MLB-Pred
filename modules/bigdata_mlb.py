...
class MLBDataWarehouse:
    ...
    def _games_fingerprint(games: pd.DataFrame) -> str:
        """Stable digest of fields that determine chronological features/targets."""
        ...
        if "Date" in x.columns:
            x["Date"] = pd.to_datetime(x["Date"], errors="coerce").dt.strftime("%Y-%m-%dT%H:%M:%S")
        raw = x.to_csv(index=False, lineterminator="\n").encode("utf-8")
    ...
    def ensure_fresh_from_repository(self, games_path: str | Path = "data/mlb_games.csv") -> dict:
        """Synchronize DuckDB/Parquet whenever the canonical historical CSV changed.

        This detects new rows and historical corrections (scores, teams, dates), not
        merely file existence. It intentionally compares only fields that affect the
        chronological feature store, avoiding unnecessary rebuilds for irrelevant CSV
        formatting changes.
        """
        path = Path(games_path)
        if not path.exists():
            return {"fresh": False, "rebuilt": False, "reason": "source_missing"}
        source = self._normalize_games(pd.read_csv(path))
        source_fp = self._games_fingerprint(source)
        con = self.connect()
        try:
            try:
                stored = con.execute("SELECT * FROM games ORDER BY Date, game_key").df()
            except Exception:
                stored = pd.DataFrame()
        finally:
            con.close()
        stored_fp = self._games_fingerprint(self._normalize_games(stored)) if not stored.empty else "empty"
        features_ok = self.paths.features.exists()
        if source_fp == stored_fp and features_ok:
            return {"fresh": True, "rebuilt": False, "games": len(source), "fingerprint": source_fp}
        ingested = self.ingest_games(source)
        rebuilt = self.rebuild_feature_store()
        return {
            "fresh": True, "rebuilt": True, "games": len(source), "ingested": ingested,
    ...
    def training_frame(self, cutoff_date: Optional[str | pd.Timestamp] = None) -> pd.DataFrame:
        ...
        try:
            ...
            return con.execute(
                "SELECT * FROM pregame_features WHERE Date < ? ORDER BY Date, game_key",
    ...
    def legacy_ml_training_frame(self, df_batting: pd.DataFrame, df_pitching: pd.DataFrame, bat_scale=100.0, pit_scale=4.10) -> pd.DataFrame:
        ...
        Seasonal stats are joined from season-1 only. Before materializing the frame,
        the warehouse is reconciled with the canonical historical CSV so a persistent
        Streamlit runtime cannot silently train on stale Big Data.
        """
        self.ensure_fresh_from_repository()
        f = self.training_frame().copy()
        if f.empty:
            return f
        bcol = batting_metric(df_batting)
        pcol = pitching_metric(df_pitching)
        if not bcol or not pcol:
            return pd.DataFrame()
        b = df_batting[["Team", "Season", bcol]].copy()
        p = df_pitching[["Team", "Season", pcol]].copy()
        b["Team"] = b["Team"].map(normalize_team)
        p["Team"] = p["Team"].map(normalize_team)
        b["Season"] = pd.to_numeric(b["Season"], errors="coerce")
        p["Season"] = pd.to_numeric(p["Season"], errors="coerce")
        b[bcol] = pd.to_numeric(b[bcol], errors="coerce")
        p[pcol] = pd.to_numeric(p[pcol], errors="coerce")
        bd = b.dropna().set_index(["Team", "Season"])[bcol].to_dict()
        pdict = p.dropna().set_index(["Team", "Season"])[pcol].to_dict()
        ...
        f["home_offense"] = off_h
        f["away_offense"] = off_a
        f["home_pitching"] = pit_h
        f["away_pitching"] = pit_a
        f["home_field"] = 1.0
        return f
    ...
    def settle_prediction(self, prediction_id: str, result: str, profit_units: float) -> bool:
        ...
        if result not in {"WIN", "LOSS", "PUSH", "VOID"}:
            raise ValueError("result debe ser WIN/LOSS/PUSH/VOID")
        con = self.connect()
        try:
            self._ensure_tracking_tables(con)
            con.execute(
                "UPDATE predictions SET settled=TRUE,result=?,profit_units=?,settled_at=CURRENT_TIMESTAMP WHERE prediction_id=?",
                [result, float(profit_units), str(prediction_id)],
            )
            return con.execute(
                "SELECT COUNT(*) FROM predictions WHERE prediction_id=? AND settled=TRUE", [str(prediction_id)]
            ).fetchone()[0] == 1
        finally:
            con.close()
    ...
    def performance_summary(self, accepted_only=True) -> dict:
        ...
        try:
            self._ensure_tracking_tables(con)
            where = "settled=TRUE" + (" AND accepted=TRUE" if accepted_only else "")
            ...
            return {
                "settled": int(n or 0), "wins": int(w or 0), "losses": int(l or 0),
                "profit_units": round(float(profit or 0), 4),
    ...
    def status(self) -> dict:
        ...
        try:
            ...
            return {
                "backend": "duckdb", "db": str(self.paths.db), "feature_version": FEATURE_VERSION,
