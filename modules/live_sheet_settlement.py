...
from .google_sheets_ledger import (
    SHEET_HEADERS,
    _google_credentials,
    _sheet_id,
    _worksheet_name,
...
def _official_final_score(game_pk: int, timeout: int = 12):
    ...
    if abstract != "final" and detailed not in {"final", "game over", "completed early"}:
        return None
...
def _load_sheet_rows(config=None):
    ...
    if not sheet_id:
        return [], {"ok": False, "configured": False, "message": "GOOGLE_SHEETS_ID not configured"}
    credentials, auth_source = _google_credentials(config)
    import gspread

    client = gspread.authorize(credentials)
    book = client.open_by_key(sheet_id)
    ws = book.worksheet(_worksheet_name(config))
    values = ws.get_all_values()
    if not values:
        return [], {"ok": True, "configured": True, "auth_source": auth_source, "message": "empty sheet"}
    header = values[0]
    if header != SHEET_HEADERS:
        return [], {"ok": False, "configured": True, "auth_source": auth_source, "message": "sheet header mismatch"}
    rows = []
    for raw in values[1:]:
...
def settle_pending_sheet(config=None, max_rows: int = 250):
    """Settle pending picks directly from Google Sheets using official MLB final scores.

    This intentionally does not depend on data/picks_ledger.csv, because Cloud Run's
    ephemeral filesystem and an absent GitHub token must not prevent settlement.
    """
    try:
        rows, status = _load_sheet_rows(config)
        if not status.get("ok"):
            return {**status, "checked": 0, "settled": 0}
        ...
        for row in pending:
            ...
            try:
                if key not in score_cache:
                    score_cache[key] = _official_final_score(key)
            ...
            if settled is None:
                continue
            result, profit_units, result_value, profit_mxn = settled
            updated = dict(row)
            updated["result_status"] = result
        ...
        sync_status = sync_rows(settled_rows, config) if settled_rows else {
            "ok": True, "configured": True, "inserted": 0, "updated": 0, "message": "nothing final yet"
        }
        return {
            ...
            "updated": int(sync_status.get("updated", 0) or 0),
        ...
        return {
            "ok": False,
            "configured": bool(_sheet_id(config)),
            "checked": 0,
            "settled": 0,
            "updated": 0,
