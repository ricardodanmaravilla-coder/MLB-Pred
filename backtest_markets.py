...
def main():
    ...
    bc = 'OPS_Index' if 'OPS_Index' in bat.columns else 'wRC+'; pc = 'ERA' if 'ERA' in pit.columns else 'xFIP'
    ...
    bd = b.dropna(subset=['K','Season',bc]).set_index(['K','Season'])[bc].to_dict(); pdic = p.dropna(subset=['K','Season',pc]).set_index(['K','Season'])[pc].to_dict()
    ...
    for _, r in test.iterrows():
        ...
        tr = pr['Proyeccion_Carreras']; st = pr['Sigma_Carreras']; po = cdf((tr-8.5)/st); pu = cdf((8.5-tr)/st)
    ...
    out={
      'n_train':len(train),'n_test':len(test),'walk_forward_state_updates':True,
      ...
      'sigma_runs':round(m.sigma_runs,3),'sigma_diff':round(m.sigma_diff,3),'historical_odds_available':False,'roi_claim_allowed':False
    }
