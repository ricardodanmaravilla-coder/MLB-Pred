...
def test_training_and_live_signal_vectors_match_exactly():
    ...
    train=build_advanced_signal_frame(frame,bat,pit).iloc[0].to_numpy(float); live=build_live_signal_row('NYY','BOS',2026,bat,pit)
...
def test_signal_direction_is_semantically_correct():
    ...
    assert d.home_woba_rel>d.away_woba_rel; assert d.home_k_rel>d.away_k_rel; assert d.home_fip_rel>d.away_fip_rel; assert d.home_kbb_rel>d.away_kbb_rel


def test_missing_signal_is_neutral_not_fabricated():
    bat,pit=_frames(); bat=bat.drop(columns=['Barrel%']); pit=pit.drop(columns=['SIERA']); d=build_advanced_signal_frame(pd.DataFrame([{'Home':'NYY','Away':'BOS','Season':2026}]),bat,pit).iloc[0]
    assert d.home_barrel_rel==1.0 and d.away_barrel_rel==1.0; assert d.home_siera_rel==1.0 and d.away_siera_rel==1.0


def test_coverage_is_explicit():
    bat,pit=_frames(); report=coverage_report(bat,pit); assert report['batting:wOBA']==1.0; assert report['pitching:xFIP']==1.0
...
def test_bullpen_schedule_load_is_small_and_directional():
    ...
    for i,day in enumerate([0,1,2]):
        rows.append({'GameID':9000+i,'Date':base+pd.Timedelta(days=day),'Season':2026,'GameType':'R','Away':'BOS','Home':'NYY','Away_Score':3,'Home_Score':4})
    factor,rest,recent,source=_team_schedule_load(pd.DataFrame(rows),'NYY')
    assert source=='completed_schedule_proxy'; assert 0.975<=factor<=1.035; assert rest==0; assert recent>=2; assert factor>1.0
